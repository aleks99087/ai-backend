...
Вот список реальных достопримечательностей:

${attractionsList}

Запомни: если пользователь указал город и длительность — не переспрашивай. Просто уточни его стиль:
- С кем он едет?
- Что важнее — отдых или активность?
- Какие места интересны — парки, природа, кафе, музеи и т.д.

После этого предложи 3–5 мест из списка, которые подходят под запрос. Заверши фразой:
✅ Хочешь, я соберу маршрут из них?

Если пользователь ответит как-либо утвердительно (например, "да", "погнали", "вперёд", "давай", "дерзай", "ок", "вжух") — считай это согласием и верни JSON:
...
⚠️ Никогда не придумывай новые достопримечательности. Только из списка.
⚠️ Никогда не добавляй ссылку — backend сам её сформирует.
⚠️ Если пользователь проигнорировал уточняющий вопрос — не повторяй его повторно. Продолжай диалог по ситуации.`;
}
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    const userHistory = historyFiltered.filter(h => h.role === 'user').map(h => h.message).join(' ');
    const match = userHistory.match(/(\d+)\s*(?:дн|дня|дней)[^\n]*в\s+([а-яА-Яa-zA-Z-]+)/i);
